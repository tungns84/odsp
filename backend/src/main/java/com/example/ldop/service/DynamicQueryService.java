...
import com.example.ldop.security.QueryValidator;
...
public class DynamicQueryService {
    ...
    public List<Map<String, Object>> executeQuery(UUID dataEndpointId, int page, int size) {
        String tenantId = com.example.ldop.config.TenantContext.getTenantId();

        // Cache: Get DataEndpoint metadata from cache or DB
        String metadataCacheKey = tenantId + ":" + dataEndpointId;
        ...
        if (endpoint.getConnector().getStatus() != com.example.ldop.domain.ConnectorStatus.APPROVED) {
            throw new IllegalStateException("Connector is not approved: " + endpoint.getConnector().getName());
        }

        DataSource dataSource = dataSourceManager.getDataSource(endpoint.getConnector().getId());
        ...
        try {
            ...
            String schema = getSchemaFromConfig(endpoint.getConnector().getConfig());
    ...
    public TestQueryResult executeTestQuery(
            com.example.ldop.domain.Connector connector,
            QueryDefinition queryDef
    ) {
        if (connector.getStatus() != com.example.ldop.domain.ConnectorStatus.APPROVED) {
            throw new IllegalStateException("Connector is not approved: " + connector.getName());
        }

        DataSource dataSource = dataSourceManager.getDataSource(connector.getId());
        Jdbi jdbi = Jdbi.create(dataSource);
        ...
        // Get schema from connector config
        String schema = getSchemaFromConfig(connector.getConfig());
     ...
     */
    public void invalidateCache(UUID dataEndpointId, String tenantId) {
        String metadataCacheKey = tenantId + ":" + dataEndpointId;
